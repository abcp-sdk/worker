...
1. **filesvc: no workspace-root containment.** A relative path resolves under
   ...
   anywhere in its own container. Symlinks are created verbatim by
   `SyncFolder` (no `pathEqual`/`hasRootPrefix` escape checks).
2. **Default workspace is `~/workspace`** (was `~/EasyLab/workspace`).
3. **Dockerfile** takes proxy as build args (no baked proxy), keeps the
   official Alpine CDN (no mirror swap), and creates `/root/workspace`.
4. **Registry defaults to the in-cluster `git.agent.svc.cluster.local`**
   (`abcp/worker`) and buildkitd to the agent namespace.

`upstream` is fetch-only (pushes disabled).
