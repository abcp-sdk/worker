...
# xa11y computer-use CLI, a self-contained Mach-O binary (built from source in
# a throwaway macOS guest by scripts/build-xa11y.sh macos).
XA11Y_BIN="${XA11Y_BIN:-${ROOT}/dist/xa11y-darwin-amd64}"
BUILDCTL="${BUILDCTL:-$(command -v buildctl || true)}"

for f in "${DIR}/Containerfile" "${DIR}/00-token.conf" "${DIR}/start.sh" "${DISK}" "${WORKER_BIN}" "${XA11Y_BIN}"; do
...
cp "${XA11Y_BIN}" "${WORK}/xa11y-darwin"
