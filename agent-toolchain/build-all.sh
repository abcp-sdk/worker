#!/usr/bin/env bash
# Build EVERY agent-toolchain image ONE AT A TIME (never in parallel), each
# with its own redirected log. Run detached and poll:
#
#   setsid bash agent-toolchain/build-all.sh > /tmp/wsbuild/all.log 2>&1 < /dev/null &
